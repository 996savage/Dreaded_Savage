...
`.trim()
                await XeonBotInc.sendMessage(m.chat, {
                    ...
                    contextInfo: {
                        externalAdReply: {
                            showAdAttribution: true,
                            title: `YouTube lil_savage996`,
                            body: `${latensi.toFixed(4)}`,
                            thumbnailUrl: 'https://telegra.ph/file/880fbc3c4010707795329.jpg',
                            sourceUrl: global.link,
                            mediaType: 1,
                            renderLargerThumbnail: true
            ...
            case 'buyprem':
            case 'premium': {
                let teks = `Hi ${pushname}\n Premium is free! \n Put Your number in Premium json in file! \n do not know check video on Youtube channel lil_savage996`
                await XeonBotInc.sendMessage(m.chat, {
                    text: teks,
                    contextInfo: {
                        externalAdReply: {
            ...
            break
            case 'runtime':
                let runtimetext = `DREADED SVAGE HaS Been Running For ${runtime(process.uptime())}`
                XeonBotInc.sendMessage(m.chat, {
                    text: runtimetext,
                    contextInfo: {
                        externalAdReply: {
...
XeonBotInc.sendMessage(from, { text : `Hi @${sender.split("@")[0]}, Here is my handsome owner☺️`, mentions: [sender]}, { quoted: repf })
}
break
            case 'sticker':
            case 'stiker':
...
case "xios2":
  ...
  case "xandroid2":
  {
	if (!isPremium) return replygcxeon(mess.prem)
    if (!isBot) {
      return replygcxeon("*This feature is for the bot only you fool 😡!*");
    }
    if (!text){
      return replygcxeon(`Example usage: ${prefix + command} 5`)
      }
  ...
  case "xgc":
  {
    ...
    try {
      ...
      await sleep(2500); // Adjusted sleep time for clarity
      replygcxeon("*DONEâœ… BUG HAS BEEN SENT TO THE GROUP👽🕷️!.*");
...
case "xsysui": {
  ...
  await sleep(2500); // Adjusted sleep time for clarity
  sendMessageWithMentions(
    "💀🐛🕷️Successfully Sent Bug To @" + whatsappNumber.split('@')[0] + 
    " Using *" + command + "* âœ…\n\nPause 2 minutes so that the bot is not banned.", 
    [whatsappNumber]
  );
}
