   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +254709137213")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Type y-our WhatsApp number mother fucker 😡\nFor example: +254709137213 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            ...
            phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Type your WhatsApp number mother fucker 😡\nFor example: +254709137213 : `)))
            phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
            rl.close()
         }
      }
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
			...
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ Dreaded Savage ᵇᵘᵍ ᵇᵒᵗ ]`)}\n\n`))
            console.log(chalk.cyan(`< ================================================== >`))
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: lil_savage996`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: t996savage `))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @liljay_savage996 `))
            console.log(chalk.magenta(`${themeemoji} WA NUMBER: +254795118507`))
            console.log(chalk.magenta(`${themeemoji} CREDIT: Liljay_savage\n`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
